...
namespace SINS
{
    public partial class frmAdmusuarios : Form
    {
        public frmAdmusuarios(Utiles.DbItem tarea, int userId)
        {
            ...
            tiposUsuairos = tu.ObtenerUsuarios();
            ...
            {
                prepararParaEditar();
                lklblPermisos.Enabled = true;
            }
        }
        string permisos_usuario;
        ...
        void asignarDatosUsuarioActual()
        {
            ...
            foreach (TipoUsuario tu in tiposUsuairos)
            {
                if (tu.Titulo == cbbxTipoUsuario.Text)
                {
                    ...
                    if (string.IsNullOrEmpty(permisos_usuario))
                    {
                        permisos_usuario = tu.Permisos;
                    }                    
                }
            }
            usuarioActual.Permisos = permisos_usuario;
        }
        ...
        void cargarInformacionDelUsuario()
        {
            if (usuarioActual.Id != 0)
            {
                if (usuarioActual.buscarDatos(Utiles.DbOptionSelect.Todo))
                {
                    ...
                    //
                    foreach (TipoUsuario item in tiposUsuairos)
                    {
                        if (item.Id == usuarioActual.Tipo_usuario)
                        {
                            cbbxTipoUsuario.Text = item.Titulo;
                        }
                    }
                }
            }
        ...
        private void lklblPermisos_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            frmPermisos frm = new frmPermisos(this.usuarioActual.Permisos,usuarioActual.Tipo_usuario);
            frm.ShowDialog();
            if (!string.IsNullOrEmpty(frm.Permisos))
            {
                usuarioActual.Permisos = frm.permisos;
                permisos_usuario = frm.permisos;
                MessageBox.Show("Los permisos del usuario han cambiado", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }

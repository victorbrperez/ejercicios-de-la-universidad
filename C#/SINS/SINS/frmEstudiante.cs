...
namespace SINS
{
    ...
    {
        public frmEstudiante(Utiles.DbItem tarea, int id_estudiante)
        {
            InitializeComponent();
        ...
        void prepararParaNuevo()
        {
            ...
            limpiarCampos(ref gbxDatosCentro);
        ...
        void inicializarComboBoxes()
        {
            ...
            tiposEstudiantes = te.traerTodosBasico();
            foreach (TipoEstudiante item in tiposEstudiantes)
            {
        ...
        {
            asignarDatos();
            if (0 == validarCamposVacios(ref gbxDatosPersonales) && 0 == validarCamposVacios(ref gbxDatosCentro))
            {
                asignarDatos();
        ...
        {
            tbxApellidos.BackColor = Color.White;
            cambios_realizados = true;
        }
        ...
        {
            prepararParaModificar();
            frmBuscarEstudiante frmBuscar = new frmBuscarEstudiante(Utiles.OpcionBusqueda.BusqedaYEnvio);
            frmBuscar.ShowDialog();
            if (frmBuscar.IdEstudianteSeleccionado > 0)
            {
                estudianteActual.Id = frmBuscar.IdEstudianteSeleccionado;
                cargarDatos(estudianteActual.Id);
            }
            frmBuscar.Dispose();
        }

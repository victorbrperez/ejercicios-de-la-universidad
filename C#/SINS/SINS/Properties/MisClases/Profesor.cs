    ...
    public class Profesor
    {
        ...
        public bool buscarDatosPorId(Utiles.DbOptionSelect option)
        {
            ...
            if (this.Id != 0)
            {
                ...
                {
                    sql.AppendFormat("SELECT Id,Nombre,Apellidos,Cedula,TelefonoMovil,TelefonoCasa,Sexo,Direccion,Correo,FechaNacimiento,EstadoCivil,Observaciones FROM tblProfesor WHERE Id = {0} AND Activo = 1; ", this.Id);
                }
                else if (option == Utiles.DbOptionSelect.Basico)
                {
                    sql.AppendFormat("SELECT SELECT Id,Nombre,Apellidos,Cedula FROM tblProfesor WHERE Id = {0} AND Activo = 1", this.Id);
                }

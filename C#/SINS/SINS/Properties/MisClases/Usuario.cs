...
namespace SINS.MisClases
{
    public class Usuario
    {
        ...
        public bool Registrar()
        {
            ...
            sql.Append("INSERT INTO tblUsuario(Usuario,Password,Nombre,Apellidos,Telefono,Tipo_usuario) ");
            sql.AppendFormat("VALUES ('{0}','{1}','{2}','{3}','{4}',{5})", this.Nombre_usuario, this.Password, this.Nombre, this.Apellidos, this.Telefono, this.Tipo_usuario);
        ...
        public bool buscarDatos(Utiles.DbOptionSelect option)
        {
            ...
            if (this.Id != 0)
            {
                ...
                {
                    sql.AppendFormat("SELECT Id,Usuario,Password,Nombre,Apellidos,Telefono,Tipo_usuario FROM tblUsuario WHERE Id = {0} AND Activo = 1", this.Id);
                }
                else if (option == Utiles.DbOptionSelect.Basico)
                {
                    sql.AppendFormat("SELECT Id,Usuario,Password,Nombre,Apellidos FROM tblUsuario WHERE Id = {0} AND Activo = 1", this.Id);
                }
            ...
            else if (!string.IsNullOrEmpty(this.Nombre_usuario))
            {
                if (option == Utiles.DbOptionSelect.Todo)
                {
                    sql.AppendFormat("SELECT Id,Usuario,Password,Nombre,Apellidos,Telefono,Tipo_usuario FROM tblUsuario WHERE Usuario = '{0}' AND Activo = 1", this.Nombre_usuario);
                }
                else if (option == Utiles.DbOptionSelect.Basico)
                {
                    sql.AppendFormat("SELECT Id,Usuario,Password,Nombre,Apellidos FROM tblUsuario WHERE Usuario = '{0}' AND Activo = 1", this.Nombre_usuario);
                }
        ...
        public bool Actualizar()
        {
            StringBuilder sql = new StringBuilder();
            SqlMan sqlman = new SqlMan();
            sql.AppendFormat("UPDATE tblUsuario SET Usuario = '{0}',Password = '{1}',Nombre = '{2}',Apellidos = '{3}',Telefono = '{4}',Tipo_usuario = {5} WHERE Id = {6}",this.Nombre_usuario,this.Password,this.Nombre,this.Apellidos,this.Telefono,this.Tipo_usuario,this.Id);

...
namespace SINS.MisClases
{
    public class Estudiante
    {
        ...
        public DataTable ObtenerListado()
        {
            StringBuilder sql = new StringBuilder();
            SqlMan sqlman = new SqlMan();
            sql.Append(@"SELECT Id,Matricula,Nombre,Apellidos,FechaNacimiento,Cedula FROM tblEstudiante WHERE Activo = 1;");
        ...
        public bool buscarDatosPorId(Utiles.DbOptionSelect option)
        {
            ...
            if (this.Id != 0)
            {
                ...
                {
                    sql.AppendFormat("SELECT Id,Matricula,Nombre,Apellidos,Cedula,Balance FROM tblEstudiante WHERE Id = {0} AND Activo = 1", this.Id);
                }
            ...
            if (!string.IsNullOrEmpty(sql.ToString()))
            {
                if (sqlman.cargarReader(sql.ToString()))
                {
                    if (sqlman.reader.Read())
                    {
                        ...
                        this.Balance = Convert.ToDouble(sqlman.reader["Balance"].ToString());
        ...
        public bool ActualizarBalance()
        {
            StringBuilder sql = new StringBuilder();
            SqlMan sqlman = new SqlMan();
            sql.AppendFormat("UPDATE tblEstudiante SET Balance = {0} WHERE Id = {1}", this.Balance, this.Id);
            return sqlman.actualizar(sql.ToString());

        }
        /// <summary>
        /// Opbtiene le Id del estudiante mediante su matricula
        /// </summary>
        /// <param name="option"></param>
        /// <returns></returns>
        public bool buscarDatosPorMatricula()
        {
            bool r = false;
            StringBuilder sql = new StringBuilder();
            SqlMan sqlman = new SqlMan();
            if (this.Matricula != 0)
            {
                sql.AppendFormat("SELECT Id FROM tblEstudiante WHERE Matricula = {0} AND Activo = 1; ", this.Matricula);
                if (sqlman.cargarReader(sql.ToString()))
                {
